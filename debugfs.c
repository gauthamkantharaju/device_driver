...
#if 0
static ssize_t db_write(struct file *filp, const char __user *buf, size_t count,
		loff_t *fpos)
{
	return simple_write_to_buffer(wrbuf, sizeof(CNTXT), fpos,
		buf, sizeof(CNTXT));
}
#endif

const struct file_operations db_fops = {
	.read = db_read,
};

#if 0
const struct file_operations db_fops1 = {
	.write = db_write,
};
#endif
...
struct dentry *debugfs_create(void)
{
	...
	if (!parent) {
		pr_alert(KERN_ALERT"Failed to create debugfs directory\n");
		parent = NULL;
	} else{
		...
		db_struct->rindex = 1;
		db_struct->windex = 1;
		debugfs_create_file("db_struct", 0666,
			parent, db_struct, &db_fops);
	}
